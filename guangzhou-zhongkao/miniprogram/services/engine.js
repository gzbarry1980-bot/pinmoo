...
const BATCH_LIMITS = { 2: 3, 3: 6, 4: 6 };
...
function gradientIndex(score, line) {
  if (!line) return 99;
...
function evaluateRecord(profile, slot, record, candidateScore, lines, tieRank = null) {
...
function replayPlan(profile, plan, dataset) {
...
function simulateOutcomes(profile, plan, dataset, seed = 20260722, iterations = 10000) {
...
function schoolFamilyKey(schoolName) {
  return String(schoolName || '').replace(/[（(].*$/u, '').replace(/\s+/g, '').trim();
}

function evaluatePlan(profile, plan, analysis) {
...
module.exports = { YEAR_WEIGHTS, BATCH_LIMITS, gradientIndex, candidateTypeFor, evaluateRecord, replayPlan, simulateOutcomes, schoolFamilyKey, evaluatePlan };
