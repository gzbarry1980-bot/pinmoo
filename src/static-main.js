...
function Footer() {
  ...
  return '<footer class="site-footer"><div class="container footer-grid"><div class="footer-brand">' + logo('logo-frame-footer') + '<p>' + SITE.company + '</p><p>专注电商战略咨询与品牌增长陪跑</p></div><div><h2>导航链接</h2><div class="footer-links">' + links + '</div></div><div><h2>联系方式</h2><p>' + SITE.contactLabel + '</p><p>' + SITE.contactNote + '</p><p>ICP备案号：待补充</p></div></div><div class="footer-bottom">© 2026 ' + SITE.company + '. All rights reserved.</div></footer>';
}
...
function Contact() {
  const contactItems = [['公司名称', SITE.company, 'Building2'], ['微信 / 手机同号', SITE.phoneDisplay, 'Phone'], ['联系说明', SITE.contactNoteWithSite, 'MessageCircle'], ['工作时间', SITE.workTime, 'Clock'], ['响应说明', SITE.responseTime, 'Send']].map(function(item) { return '<div class="contact-info-row"><span>' + icon(item[2], 26) + '</span><div><h2>' + item[0] + '</h2>' + (item[0].indexOf('手机') >= 0 ? '<a href="tel:' + SITE.phone + '">' + item[1] + '</a>' : '<p>' + item[1] + '</p>') + '</div></div>'; }).join('');
...
function initContactForm() {
  ...
  const servicesInput = form.querySelector('.form-services');
  form.querySelectorAll('[data-platform]').forEach(function(btn) {
    btn.addEventListener('click', function() {
      if (selected.has(btn.dataset.platform)) selected.delete(btn.dataset.platform); else selected.add(btn.dataset.platform);
      btn.classList.toggle('selected');
      if (platformInput) platformInput.value = Array.from(selected).join(' / ');
    });
  });
  form.querySelectorAll('[data-service]').forEach(function(btn) {
    btn.addEventListener('click', function() {
      if (selectedServices.has(btn.dataset.service)) selectedServices.delete(btn.dataset.service); else selectedServices.add(btn.dataset.service);
      btn.classList.toggle('selected');
  ...
  form.addEventListener('submit', async function(event) {
    ...
    if (!String(fd.get('name') || '').trim()) errors.name = isEn() ? 'Please enter your name' : '请填写姓名';
    if (!String(fd.get('company') || '').trim()) errors.company = isEn() ? 'Please enter your company name' : '请填写公司名称';
    if (!phone) errors.phone = isEn() ? 'Please enter your phone number' : '请填写联系电话';
    else if (!/^1[3-9]\d{9}$|^0\d{2,3}-?\d{7,8}$/.test(phone)) errors.phone = isEn() ? 'Please enter a valid phone number' : '请输入正确的联系电话';
    ...
    submit.textContent = isEn() ? 'Submitting...' : '提交中...';
    success.hidden = true;
