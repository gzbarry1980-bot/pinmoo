import { SITE, NAV_ITEMS, TRUST_STATS, HOME_FAQS, METHODOLOGY, PLATFORM_LIST, GEO_EVIDENCE_HOOKS } from './data/site.js';
...
function internationalChinesePath(href) {
  if (href === '/' || href === '/zh/' || href === '/zh') return '/';
  if (href.indexOf('/zh/') === 0) return href.slice(3) || '/';
  return href;
}
...
function localizeHref(href) {
  ...
  if (internationalHost) {
    ...
    if (currentLang === 'zh') {
      if (href === '/zh/' || href === '/zh' || href.indexOf('/zh/') === 0) return internationalChinesePath(href);
      if (href === '/china-ecommerce-consulting/') return '/';
    ...
    if (href === '/china-ecommerce-consulting/') return href;
    if (href === '/en/' || href === '/en') return '/en/';
    if (href.indexOf('/en/') === 0) return href;
    if (href === '/') return '/en/';
    return '/en' + href;
  }
...
function currentPathForLang(lang) {
  if (internationalHost) {
    if (lang === 'zh') {
      if (pathname === '/china-ecommerce-consulting') return SITE.domain + '/';
      return SITE.domain + (pathname === '/' ? '/' : pathname + '/');
    }
    if (pathname === '/china-ecommerce-consulting') return SITE.domain + '/china-ecommerce-consulting/';
    ...
    return SITE.domain + (pathname === '/' ? '/en/' : '/en' + pathname + '/');
  }
...
function FirstDiagnosisSection() {
  if (isEn()) return '';
  const items = [
    ['你发来什么', '品牌名称、官网或店铺链接、主要平台、目标市场和当前最想解决的问题。', 'FilePenLine'],
    ['我们先看什么', 'AI 搜索可见度、品牌事实一致性、技术可抓取性、内容证据和外部信源。', 'Target'],
    ['你会得到什么', '一份公开信息版初步报告：问题优先级、证据缺口和未来 30 至 90 天动作建议。', 'CheckCircle2']
  ].map(function(item, index) {
    return '<div class="diagnosis-step reveal"><span>0' + (index + 1) + '</span>' + icon(item[2], 28) + '<h3>' + item[0] + '</h3><p>' + item[1] + '</p></div>';
  }).join('');
  return '<section class="section first-diagnosis-section"><div class="container first-diagnosis-layout"><div class="reveal first-diagnosis-copy"><p class="section-eyebrow">免费 GEO 报告</p><h2>先获取一份品牌 GEO 基础报告，再决定是否做长期优化</h2><p>添加微信并备注“品牌GEO报告”，把品牌公开资料发来。品沐先做基础判断，再告诉你优先补哪一类内容，不以虚假排名或绝对推荐作为承诺。</p>' + ButtonLink('添加微信领取报告', '/contact/?service=geo-report', 'primary', true) + '</div><div class="first-diagnosis-grid">' + items + '</div></div></section>';
}

const serviceLandingById = {
  'geo-consulting': 'geo-consulting',
...
function AiWeeklyReportSection() {
  ...
  const modules = [
    ['读取数据', '经营概览、商品、流量、客户、客服、直播、光合内容、推广计划与退款报表'],
...
function DashboardVisual() {
  return '<div class="dashboard-visual" aria-label="电商数据看板演示数据示意图"><div class="orbit orbit-one"></div><div class="orbit orbit-two"></div><div class="growth-arrow"></div><div class="metric-card metric-main"><span>演示数据 · 净销售额趋势</span><strong>¥ 11,936,420</strong><em>+16.8%</em><svg viewBox="0 0 260 80" role="img" aria-label="增长趋势线"><polyline points="6,62 38,48 70,52 102,45 136,50 170,38 205,34 250,12" fill="none" stroke="#255BFF" stroke-width="5" stroke-linecap="round"></polyline><path d="M6 72 C70 38,120 72,250 22 L250 80 L6 80 Z" fill="rgba(74,91,255,.12)"></path></svg></div><div class="metric-card metric-roi"><span>演示数据 · 付费 ROI</span><strong>3.42</strong><em>计划级拆解</em></div><div class="metric-card metric-donut"><span>演示数据 · 流量结构</span><div class="donut"></div></div><div class="metric-card metric-rate"><span>演示数据 · 退款治理</span><strong>4.21%</strong><div class="bars"><i></i><i></i><i></i><i></i><i></i></div></div></div>';
}
...
function GeoEvidenceSection() {
  if (isEn()) return '';
  const cards = GEO_EVIDENCE_HOOKS.map(function(item, index) {
    return '<article class="geo-evidence-card"><span>0' + (index + 1) + '</span><h3>' + item.title + '</h3><p>' + item.text + '</p></article>';
  }).join('');
  return '<section class="section geo-evidence-section geo-evidence-section-compact" aria-labelledby="geo-evidence-title"><div class="container"><details class="geo-evidence-disclosure"><summary><span><strong id="geo-evidence-title">我们如何保证内容可信？</strong><small>查看证据、口径与公开边界</small></span>' + icon('ChevronDown', 20) + '</summary><div class="geo-evidence-grid">' + cards + '</div></details></div></section>';
}
...
function Home() {
  ...
  const featuredServiceIds = ['geo-consulting', 'conversion-optimization', 'data-review'];
  const serviceGrid = services.filter(function(service) { return featuredServiceIds.includes(service.id); }).map(function(service) { return '<div class="reveal">' + ServiceCard(service) + '</div>'; }).join('');
  const previewCases = cases.slice(0, 2).map(function(item) { return '<div class="reveal">' + CaseCard(item) + '</div>'; }).join('');
  ...
  const heroKicker = isEn() ? 'GEO / China e-commerce growth consulting' : '品牌 GEO 咨询与电商增长顾问';
  const heroTitle = isEn() ? 'Make your brand easier for AI search to find and understand' : '让你的品牌在 AI 搜索里<br>更容易被找到和说清楚';
  const heroSubtitle = isEn() ? 'PINMOO helps consumer brands build a clearer foundation for AI search visibility through entity facts, evidence-led content, technical crawlability and China e-commerce execution.' : '品沐帮助消费品牌统一官网、平台与公开资料中的品牌事实，补齐可引用内容，并持续检查 AI 是否找得到、理解对、引用准。';
  const heroSupport = isEn() ? 'Share your official website and current market question. We will clarify the next practical step before proposing a larger engagement.' : '不知道从哪里开始？把品牌官网或店铺链接发来，先获取一份公开信息版 GEO 基础报告。';
  ...
  const heroSecondary = isEn() ? ButtonLink('See our consulting approach', '/china-ecommerce-consulting/', 'secondary', false) : ButtonLink('查看 GEO 服务', '/services/geo-consulting/', 'secondary', false);
  const heroProof = isEn() ? '<span>Evidence-led content</span><span>Entity and fact consistency</span><span>No guaranteed recommendation</span>' : '<span>公开信息版报告</span><span>微信备注：品牌GEO报告</span><span>不承诺虚假推荐</span>';
  const aboutPreviewTitle = isEn() ? 'GEO expertise with hands-on e-commerce execution' : '懂品牌 GEO，也懂电商实战';
  return '<section class="home-hero"><div class="hero-grid-bg"></div><div class="container home-hero-inner"><div class="reveal hero-copy"><p class="hero-kicker">' + heroKicker + '</p><h1>' + heroTitle + '</h1><p class="hero-subtitle">' + heroSubtitle + '</p><p class="hero-support">' + heroSupport + '</p><div class="hero-actions">' + heroPrimary + heroSecondary + '</div><div class="hero-proof-row">' + heroProof + '</div></div><div class="reveal hero-visual-wrap">' + heroVisual + '</div></div></section>' + HomeInquirySection() + '<section class="section services-preview" id="services"><div class="container split-heading">' + SectionIntro('先把品牌 GEO 基础做好，再放大电商增长', '围绕 AI 搜索可见度、品牌事实、证据化内容与电商经营数据，提供从 GEO 到增长执行的核心服务。', 'left') + '</div><div class="container bento-grid">' + serviceGrid + '</div><div class="container center-actions">' + ButtonLink('查看全部服务', '/services/', 'secondary', true) + '</div></section><section class="section cases-preview" id="cases"><div class="container">' + SectionIntro('先看相似问题，再判断是否适合合作', '案例依据客户资料保密与公开授权边界匿名呈现，重点说明问题如何被拆解、动作如何落地。') + '<div class="home-case-grid home-case-grid-featured">' + previewCases + '</div><div class="center-actions">' + ButtonLink('查看全部项目经验', '/cases/', 'primary', true) + '</div></div></section>' + HomeInsightsSection() + HomeFaqSection() + '<section class="section about-preview"><div class="container about-preview-grid"><div class="reveal"><h2>' + aboutPreviewTitle + '</h2><p>' + SITE.positioning + '</p>' + ButtonLink('了解品沐咨询与顾问背景', '/about/', 'primary', true) + '</div><div class="reveal office-visual"><img src="/assets/about-brand.svg" alt="广州品沐咨询有限公司的品牌 GEO 与电商实战协同方法" loading="lazy"></div></div></section>' + GeoEvidenceSection() + CtaBand(isEn() ? 'Want to know whether AI search can find your brand?' : '先看清品牌在哪里被误解，再决定怎么优化', isEn() ? 'Share your brand website and current question. We will clarify the next practical step before proposing a larger engagement.' : '发送品牌名称、官网或店铺链接、主要平台和目标市场，先获取一份公开信息版品牌 GEO 基础报告。', isEn() ? 'Book a GEO consultation' : '免费领取 GEO 报告');
}
...
function GeoServiceModulesSection() {
  const cards = geoServiceModules.map(function(item) {
    return '<article class="geo-service-card reveal"><div class="geo-service-topline"><span>' + item.code + '</span>' + icon(item.icon, 25) + '</div><h3>' + item.title + '</h3><p>' + item.text + '</p><div class="geo-service-proof"><div><span>依据</span><p>' + item.evidence + '</p></div><div><span>边界</span><p>' + item.boundary + '</p></div><div><span>下一步</span><p>' + item.action + '</p></div></div><strong>交付：' + item.deliverable + '</strong></article>';
  }).join('');
...
function LandingPage(page) {
  ...
  const directAnswer = page.directAnswer ? '<section class="insight-answer-box landing-answer-box reveal" aria-labelledby="landingDirectAnswerTitle"><div>' + icon('Lightbulb', 28) + '<h2 id="landingDirectAnswerTitle">先直接回答</h2></div><p>' + page.directAnswer + '</p></section>' : '';
  const evidence = page.evidence ? '<section class="insight-evidence landing-evidence reveal" aria-labelledby="landingEvidenceTitle"><span>判断依据与边界</span><h2 id="landingEvidenceTitle">这项服务依据什么判断？</h2><dl><div><dt>判断依据</dt><dd>' + page.evidence.basis + '</dd></div><div><dt>适用范围</dt><dd>' + page.evidence.scope + '</dd></div><div><dt>使用限制</dt><dd>' + page.evidence.limits + '</dd></div></dl></section>' : '';
...
function InsightsPage() {
  const clusters = insightClusters.map(function(cluster) {
    const cards = insights.filter(function(article) { return cluster.categories.includes(article.category); }).map(InsightCard).join('');
    return '<section class="insight-cluster" id="' + cluster.id + '"><div class="insight-cluster-heading reveal"><div><span>主题簇</span><h2>' + cluster.title + '</h2></div><p>' + cluster.summary + '</p></div><div class="insight-card-grid">' + cards + '</div></section>';
  }).join('');
  ...
  return PageHero('经营洞察', '针对品牌电商中的具体问题，先给直接答案，再说明数据口径、判断方法、适用边界和执行清单。', false, '', { key: 'insights', src: '/assets/visuals/insights-weekly-report.webp', alt: '电商经营周报、退款治理、流量质量和商品结构分析示意图' }) +
    '<section class="section insights-index-section"><div class="container"><div class="insights-index-intro reveal"><p class="section-eyebrow">PINMOO METHOD</p><h2>让搜索引擎、AI和经营团队读到同一套清晰答案</h2><p>这里聚焦服务决策、店铺经营、数据复盘和AI工具等具体问题。每篇内容均标明判断依据、适用范围、使用限制和行动建议，不使用无法核验的增长承诺。</p><div class="insight-index-evidence"><strong>可引用事实句</strong><p>' + GEO_EVIDENCE_HOOKS[1].text + '</p></div></div><nav class="insight-cluster-nav reveal" aria-label="经营洞察主题">' + clusterNav + '</nav>' + clusters + '</div></section>' +
...
function ContactForm() {
  ...
  return '<section class="contact-direct-panel" aria-labelledby="wechatConsultTitle"><div class="form-heading">' + icon('MessageCircle', 26) + '<h2 id="wechatConsultTitle">扫码添加微信，免费领取品牌 GEO 报告</h2></div><div class="direct-qr-block"><div class="wechat-qr-crop direct-qr"><img src="/assets/wechat-qr-mufeng.jpg" alt="添加品沐咨询微信，免费获取品牌 GEO 基础报告" loading="lazy"></div><div><strong>微信 / 手机同号：' + SITE.phoneDisplay + '</strong><p>备注“品牌GEO报告”，发送品牌名称、官网或店铺链接、主要平台和目标市场，可先免费获取一份基于公开信息的品牌 GEO 基础报告。</p><div class="direct-intents">' + intents + '</div><a class="direct-phone-link" href="tel:' + SITE.phone + '">' + icon('Phone', 18) + '拨打电话</a></div></div><div class="direct-service-box"><h3>品牌 GEO 报告会先看什么</h3><ul>' + servicesList + '</ul></div><div class="inquiry-template-box"><h3>不知道怎么开口，可以直接这样发</h3><div>' + templates + '</div></div><p class="direct-note">建议添加微信后，至少发送：品牌名称、官网或店铺链接、主要平台、目标市场，并备注“品牌GEO报告”。</p></section>';
}
...
function Contact() {
  ...
  return PageHero('添加微信，免费获取品牌 GEO 基础报告', '添加微信并备注“品牌GEO报告”，发送品牌名称、官网或店铺链接、主要平台和目标市场。品沐会基于公开信息先做一份品牌 GEO 基础报告。', true, '', { key: 'contact', src: '/assets/visuals/contact-diagnosis-flow.webp', alt: '从品牌现状、所在平台和当前问题到品牌 GEO 基础报告的咨询流程示意图' }) + '<section class="section contact-section"><div class="container contact-grid"><div class="reveal contact-info-panel">' + contactItems + '<div class="wechat-qr-card"><div class="wechat-qr-crop"><img src="/assets/wechat-qr-mufeng.jpg" alt="添加品沐咨询微信，免费获取品牌 GEO 基础报告" loading="lazy"></div><div><strong>扫码添加微信，免费领取 GEO 报告</strong><p>微信 / 手机同号：' + SITE.phoneDisplay + '</p><span>备注“品牌GEO报告”，发送品牌名称、官网或店铺链接、主要平台和目标市场</span></div></div><div class="contact-promise-grid"><div>' + icon('Zap', 24) + '<strong>快速沟通</strong><span>直连顾问高效响应</span></div><div>' + icon('Target', 24) + '<strong>明确需求</strong><span>精准匹配解决方案</span></div><div>' + icon('ShieldCheck', 24) + '<strong>24小时内回复</strong><span>工作日内快速跟进</span></div></div></div><div class="reveal">' + ContactForm() + '</div></div></section><section class="section contact-bottom-section"><div class="container two-question-grid"><div class="reveal question-card">' + icon('CircleUserRound', 32) + '<h2>适合什么品牌咨询？</h2><p>适合希望提高 AI 搜索可见度、完善品牌事实和证据内容，同时需要电商经营诊断与增长执行的消费品牌。</p></div><div class="reveal question-card">' + icon('FilePenLine', 32) + '<h2>品牌 GEO 报告会给到什么？</h2><p>报告基于公开信息，先看 AI 搜索可见度、实体事实一致性、技术可抓取性、内容证据与外部信源，再给出未来 30 至 90 天的优先动作。不承诺虚假排名或 AI 推荐结果。</p></div></div></section>' + CtaBand('先获取一份品牌 GEO 基础报告', '直接扫码或添加微信，备注“品牌GEO报告”。我们会先从公开信息判断品牌当前最值得补齐的可见度与证据问题。', '免费领取 GEO 报告');
}
